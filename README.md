# XMC MCU: CCU8 Timer Concatenation
...
The code example uses three CCU8 slices of the XMC MCU:

1. One slice configured as a timer. It generates a time base of 1 us. It will be used to count up the timer of the other two.

2. Two slices linked together to form a 32-bit counter. They use the slice comparison status signal (CC8x.STy) of the first timer to count up the timer value.
...
4. In the **Quick Panel**, scroll down, and click **\<Application Name> Program (JLink)**.

5. Confirm that the kit LED blinks at approximately 1 Hz. (P4.1 for XMC1400 and P5.8 for XMC4700)
...
2. COUNTER_H and COUNTER_L configured as counters. These two slices are linked together forming a 32-bit counter. They will increase their timer value based on the 1-us time base, using the falling edge of the slice comparison status signal (CC8x.STy) from TIMER_0.

For the external input trigger, the XMCLib Peripheral Driver for the CCU8 function offers `XMC_CCU8_SLICE_SetInput`. This is used for assigning an input signal to the count function.
...
| [AP32288 - Capture Compare Unit (CCU8)](https://www.infineon.com/dgdl/Infineon-CCU8-XMC1000_XMC4000-AP32288-AN-v01_01-EN.pdf?fileId=5546d4624e765da5014ed8dd5c7d1730) – Application Note | Describes the Capture Compare Unit (CCU8) module and provides hints for developing motor control applications with the XMC™ microcontroller family.
|
| **Code Examples**                                            |                                                              |
...
Document Title: *CE232667* - *XMC MCU: CCU8 Timer Concatenation*
